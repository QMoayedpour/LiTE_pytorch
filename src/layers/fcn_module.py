...
class InceptionModule(nn.Module):
    def __init__(self, input_channels, n_filters, dilation_rate, stride=1, kernel_size=41, activation="linear", use_hybrid_layer=False, use_multiplexing=True, device="cpu"):
    ...
    def forward(self, input_tensor):
        ...
        if self.use_hybrid_layer:
            self.hybrid = hybridLayer(input_channels=input_tensor.shape[1], device = self.device).to(self.device)
